...
class AUC:
    def __init__(self, name, kfold_fpr_tpr, fpr, tpr, mean_auc):
        ...
        plt.title(name)
        plt.legend(handles=[blue_patch, gray_patch, red_patch], loc='lower right', prop={'size': 10})
        plt.plot([0, 1], [0, 1], 'r--')
        plt.xlim([0, 1])
        plt.ylim([0, 1])
        ...
        for fprs_tpr in kfold_fpr_tpr:
            plt.plot(fprs_tpr[0], fprs_tpr[1], color="gray")
