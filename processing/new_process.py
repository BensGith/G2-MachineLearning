...
df = normal_data.apply(lambda x: x.fillna(x.mean()), axis=1)

pca = PCA(0.95)
pca.fit(df.loc[:, df.columns!='label'])
df = pd.DataFrame(pca.fit_transform(df.loc[:, df.columns!='label']))
df = pd.concat([df, label], axis=1)

# split to train, validation
train, validation = train_test_split(df)
train_label = np.array(train['label'])
validation_label = np.array(validation['label'])
print(train.shape)
print(validation.shape)



train_pca = pca.transform(train.loc[:,train.columns!='label'])
validate_pca = pca.transform(validation.loc[:,train.columns!='label'])

knn = KNeighborsClassifier(3)
knn.fit(train_pca, train_label)
predictions = knn.predict(validate_pca)
